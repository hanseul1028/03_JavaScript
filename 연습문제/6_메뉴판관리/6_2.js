...
/* #resName (span)이 클릭된 경우 #resName을 숨기고 #resNameInput을 보이게 만들기 */
...
/* #resNameInput이 포커스를 잃었을 때 (blur)
    #resNameInput을 숨기고 #resName을 보이게 만들기*/
...
updateBtn.addEventListener("click", ()=>{
    ...
    menus.forEach( item => {
        ...
        // 5) .menu 내부에 추가될 input 요소를 모아둔 JS객체 얻어오기
...
addMenu.addEventListener("click", ()=>{
    ...
    // li 요소를 생성 ( + calss명 : menu)
    const li = newEl("li", {}, ["menu"] );

    // li에 들어갈 input 3개 만들어서 객체로 반환받기
    const menuContent = createMenuContent();

    // li 에 menuContent 값을 모두 추가
    for(let key in menuContent){
