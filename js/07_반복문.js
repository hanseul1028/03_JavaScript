/*  for( [1]초기식 ; [2]조건식 ; [4]증감식 ){
        ...
        // [3] 조건식이 true일 경우 수행하는 코드;
    }

    * 처음 for문이 수행될 때만 [1] 초기식 수행
...
function check10(){
    ...
    const multiple = Number(document.getElementById("multiple10"));
    ...
    for(let num = start; num <= end; num++){
        ...
        // start 부터 end까지 1씩 증가하는 num이
        // multiple 로 부터 나눴을 때 나머지가 0인 경우
        // == num 이 multiple 의 배수인 경우
        if(num % multiple === 0){
...
function check16(){
    ...
    for(let num=1 ; num <= 30 ; num++){
        ...
        if(num % 10 === 0 ) // 10의 배수인 경우
            result += "<br>"; //줄바꿈 태그 추가
    }
    // #result16에 출력 (html 해석해야 함)
...
function check29(){
    ...
    if(dCount > 0 ){ //돈까스을 주문한 경우
        outputHtml += `<li>돈까스(${dCount}개) : ${donkkasue * dCount}원</li>`;
    }
    /* 합계 */
