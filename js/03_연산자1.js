...
전역변수로 선언하면 안될까?
  -> 안됨
  -> 전역변수는 페이지 로딩 시전에 해석되어서 값이 결정된다
...
function plusFn(){
    ...
    // Number('숫자 형태 문자열') : 숫자 형태 문자열 -> 숫자로 변경
    // 단, 숫자 형태 문자열이 아닐 경우 NaN (Not a Number) 결과반환
...
function allOperationFn(){
    // 함수가 호출되는 시점의 input 값을 얻어와 변수 저장
