...
function check1(){
    ...
    // id 가 "input1"인 요소에 작성된 값을 얻어와 숫자로 변환하여
    // val 상수에 저장

    const val = Number(in1.value);

    // 양수 (0보다 큰 수)가 맞는 지 검사
    if(val > 0){
...
// 두 수 A, B를 입력받아 A가 B의 배수가 맞는 지 확인
function check4(){
    // id가 inputA, inputB 인 요소를 얻어와 변수에 저장
    ...
    //inputA, inputB 중 하나라도 입력되지 않은 경우
...
// 매개 변수(Parameter)
// - 함수가 호출될 때 전달된 값(전달인자, Argument)을 저장하기 위한 변수
...
function check6(){
    ...
    //  -> break 미작성 시 : 현재 case 수행 후 멈추지 않고 다음 case를 연달아 수행

    // (break 또는 switch 끝 } 를 만날 때 까지 연달아 수행
