...
function check1(){
    ...
    arr2[6] = "건너뛰기도 가능?"; // 6번 인덱스에 대입됨 + 건너뛴 4,5번 인덱스가 비어있음
...
function check3(){
    ...
    const arr = []; //0칸 짜리 배열 -> 값 대입 -> 자동으로 증가
...
// 생성된 배열 값 요소에 0에서 배열 길이 미만의 난수를 저장
function check5(){
    const len = Number(prompt("생성할 배열 길이 입력"));
    const arr = new Array(len); // 입력받은 수 만큼의 칸을 가지는 배열 생성
...
function selectMenu(){
    const menus = [
        "알탕", "마라탕", "엽기떡볶이", "햄버거", "포케", "카이센동", "샌드위치", 
        "삽겹살", "우동", "우육면", "돈까스", "라면", "고등어 구이", "보쌈", "육회비빔밥"
    ];
...
function createLottoNumber(){
    ...
    for(let i = 0 ; i < 6 ; i++){ 
        ...
        if(arr.includes(num)){
            i--; // index를 뒤로 갔다가 앞으로 오게 함 -> 제자리 유지
...
function check6(){
    ...
    console.log("dArr2[1][2] : ", dArr2[1][2]); // 50
...
function check7(){
    ...
    //    2차원 배열 어디에 위치하는지 검색해서 출력하기
    // (정상 입력 했다고 가정)
    const input = Number(prompt("1 ~ 16 사이 숫자를 입력"));
