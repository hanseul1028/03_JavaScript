...
/* 고전 이벤트 모델 제거 */
// #testB 버튼 틀릭 시 #testA의 클릭 이벤트 핸들러 제거

const testB = document.querySelector("#testB");

testB.onclick = function(){
    // testA 가 클릭 되었을 때의 동작(핸들러)을 null로 덮어씌움
    // -> 이벤트 핸들러가 사라짐(제거)
    testA.onclick = null;

    alert("확인 버튼 클릯 이벤트 핸들러 제거");
 }
...
for(let i = 0 ; i < btns.length ; i++ ){
    ...
    btns[i].addEventListener("click", function(){
        ...
        // #result2의 배경색은 클릭된 커튼에 작성된 색상으로 변경
...
/* 
 ...
 const dis = document.querySelectorAll(".display"); // 출력화면 5개
 // -> 위 아래로 연결된 .display 와 .inputColor의 인덱스 번호가 같음
 ...
 changeBtn.addEventListener("click", function(){
    ...
    for(let i = 0; i < inputs.length ; i++ ){
        ...
        dis[i].style.backgroundColor = inputs[i].value;
    }
});