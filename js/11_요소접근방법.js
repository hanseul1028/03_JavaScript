...
function classTest(){
    ...
    /* 유사 배열 : 
        ...
        베열 전용 함수(메서드)를 제공하지 않음 */

    // calss 속성 값이 "cls-test"인 요소를 모두 얻어와
    // (유사) 배열 형태로 반환
...
function classTest2(){
    ...
    // id 가 sumResult인 요소의 내용으로 sum 출력
...
function cssTest(){
    ...
    // taget-div 속성 값이 css-div인 요소 하나 선택
    const container 
    ...
    for(let i = 0; i < divs.length ; i++){
        ...
        divs[i].style.justifyContent = "center"; // 중심축 방행 가운데
        divs[i].style.alignItems = "center"; // 교차축 방행 가운데
...
// #userinput 인 요소에 "Enter" 키가 입력 된 경우 채팅 추가
